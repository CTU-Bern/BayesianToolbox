...
The **ABLATIO-BILICA trial** is an example of a Bayesian phase-II trial design with interim threshold stopping using predictive probabilities (@Lee2008, @Sambucini2021). The ABLATIO-BILICA trial aims to evaluate the safety of biliary intraductal radiofrequency ablation (bRFA) in patients with unresectable extrahepatic biliary tract cancer (EBTC) undergoing standard of care chemo-immune checkpoint inhibitor-therapy (CICI). It is a multicenter two-arm randomized controlled study.

We quantify the following trial operations characteristics: Probability of stopping because of non-safety (stopping trial before
is reached), probability of treatment discontinuation and the expected number of patients in experimental arm using a simulation approach.
