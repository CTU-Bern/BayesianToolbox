# Bayesian sample size calculation for a continuous outcome
...
power_noprior <- numeric(n_simulations)
power_prior <- numeric(n_simulations)

# Simulate and fit the model multiple times to estimate power
for (i in 1:n_simulations) {
  ...
  power_noprior[i] <- posterior_prob_noprior > threshold_prob
  posterior_prob_prior <- 1 - inla.pmarginal(0, model_prior$marginals.fixed$treatment)
  power_prior[i] <- posterior_prob_prior > threshold_prob
  }
  
# Calculate average power
average_power_noprior <- mean(power_noprior)
average_power_prior <- mean(power_prior)
```


## Result of simulation

The probability of success based on the non-informative and informative prior is `r average_power_noprior` and `r average_power_prior`, respectively.
